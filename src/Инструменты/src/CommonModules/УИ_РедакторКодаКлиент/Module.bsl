		...
		ИначеЕсли ВидРедактора = ВидыРедактора.Monaco Тогда
			...
			Если ПараметрыРедактора.Язык <> "bsl" Тогда
				...
				Если ПараметрыРедактора.Язык = "bsl_query" Тогда
					...
					ДобавитьПунктМеню(ДокументView, ПараметрыРедактора, "INSERT_MACRO_COLUMN", "Вставить макроколонку");
				КонецЕсли;
			КонецЕсли;
...
//			ДокументView.setOption("generateSnippetEvent", Истина);
			...
			Если ПараметрыРедактора.ПараметрыРедактора.ИспользоватьКомандыРаботыСБуферомВКонтекстномМеню Тогда
				ДобавитьПунктМеню(ДокументView, ПараметрыРедактора, "TOOLS_UI_1C_COPY_TO_CLIPBOARD", "Копировать");
				ДобавитьПунктМеню(ДокументView, ПараметрыРедактора, "TOOLS_UI_1C_PASTE_FROM_CLIPBOARD", "Вставить");
			КонецЕсли;
...
//  ПараметрыРедактора - Структура - см.УИ_РедакторКодаКлиентСервер.НовыйДанныеРедактораФормы()
//  Идентификатор - Строка - Идентификатор команды меню
//  Наименование - Строка - Наименование команды меню
Процедура ДобавитьПунктМеню(ДокументView, ПараметрыРедактора, Идентификатор, Наименование)	
	КнопкиМенюРедактора = ПараметрыРедактора.КнопкиМенюРедактора;
	Если КнопкиМенюРедактора.Получить(Идентификатор) = Неопределено Тогда	
		ДокументView.addContextMenuItem(Наименование, Идентификатор);
		КнопкиМенюРедактора.Вставить(Идентификатор, Наименование);		
	КонецЕсли;
КонецПроцедуры
